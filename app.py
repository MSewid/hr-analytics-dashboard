...
def load_and_process_data(uploaded_file=None):
    try:
        ...
        # Fill missing Date Closed with current time
...
def forecast_requirements(monthly_req_df, months_ahead=6):
    ...
    for m in range(1, months_ahead + 1):
        t_future = last_t + m
        req_pred = max(0, round(intercept + slope * t_future))  # Round to nearest integer
...
def main():
    ...
    if "HR" in df.columns:
        ...
        selected_hr = st.sidebar.multiselect("Select HR Representatives", hr_options, default=hr_options)
    else:
        selected_hr = []
    ...
    with col2:
        st.metric("Avg Time to Fill", f"{metrics['time_to_fill']:.0f} days")
    with col3:
        st.metric("Aging of Open Req.", f"{metrics['aging']:.0f} days")
    with col4:
    ...
    if "Month" in df.columns and "Req" in df.columns:
        ...
        if forecast_df is not None:
            ...
            fig_fc.add_trace(go.Bar(x=hist["Month"], y=hist["Req"], name="Historical", marker_color='lightblue'))
            if not fut.empty:
                fig_fc.add_trace(go.Bar(x=fut["Month"], y=fut["Req"], name="Forecast", marker_color='orange'))
            fig_fc.add_trace(go.Scatter(x=forecast_df["Month"], y=forecast_df["MA_3"], name="3-Month MA", mode="lines+markers", line=dict(color='green')))
